...
vector<double> Matrix_ToolBox::Allocate_Vec_Vec_2F(int M){
...
vector<vector<double>> Matrix_ToolBox::Allocate_Mx_Vec_2F(int M, int N){
    vector<vector<double>> ret_Mx;
    vector<double>         Buff_Mx;
    for(int i = 0; i < M; ++i){
        Buff_Mx.clear();
        for(int j = 0; j < N; ++j){
            Buff_Mx.push_back(0.);
...
// Matrix Initialization & Clear
void Matrix_ToolBox::Clear_Matrix(vector<vector<int>> & Mx, int start = 0){
...
void Matrix_ToolBox::Clear_Matrix(vector<int> & Mx, int start = 0){
    for(int i = 0; i < Mx.size(); ++i){
        Mx[i] = start;
    }
}
void Matrix_ToolBox::Clear_Matrix(vector<vector<float>> & Mx, float start = 0.){
    for(int i = 0; i < Mx.size(); ++i){
        for(int j = 0; j < Mx[i].size(); ++j){
            Mx[i][j] = start;
        }
...
void Matrix_ToolBox::Clear_Matrix(vector<float> & Mx, float start = 0.){
    for(int i = 0; i < Mx.size(); ++i){
        Mx[i] = start;
    }
}
void Matrix_ToolBox::Clear_Matrix(vector<double> & Mx, double start = 0.){
    for(int i = 0; i < Mx.size(); ++i){
        Mx[i] = start;
    }
}

// Matrix Numerical Calculation
...
vector<vector<double>> Matrix_ToolBox::Matrix_Mul(vector<vector<float>> leftMx, vector<vector<double>> rightMx){
    int leftM = leftMx.size(), leftN = leftMx[0].size(), rightM = rightMx.size(), rightN = rightMx[0].size();
    if(leftN != rightM)
        throw "The dimensions doesn't match";
    vector<vector<double>> ret_Mx;
    vector<double> Buff_Vec;
    double buff_val;
    for(int i = 0; i < leftM; ++i){
        Buff_Vec.clear();
        for(int j = 0; j < rightN; ++j){
...
vector<vector<double>> Matrix_ToolBox::Matrix_Transpose(vector<vector<float>> Mx){
    vector<vector<double>> ret_Mx = Allocate_Mx_Vec_2F(Mx[0].size(), Mx.size());
    for(int i = 0; i < ret_Mx.size(); ++i){
        for(int j = 0; j < ret_Mx[i].size(); ++j){
            ret_Mx[i][j] = (double) Mx[j][i];
        }
    }
    return ret_Mx;
}
